# scoop-bucket

A [Scoop](https://scoop.sh) bucket for my tools on Windows.

```powershell
scoop bucket add redswoop https://github.com/redswoop/scoop-bucket
```

## baduhan — native Windows terminal

```powershell
scoop install baduhan
```

A fast, native Windows terminal with iTerm2-style tabs, splits, and embedded
browser panes. Baduhan (պատուհան) is Armenian for window.

Upstream project: <https://github.com/redswoop/baduhan>

## xfer — unified transfer monitor

```powershell
scoop install xfer
```

This pulls in [`aria2`](https://aria2.github.io/) automatically (xfer's download
engine). xfer manifests are generated and updated automatically by the
[xfer release workflow](https://github.com/redswoop/xfer); don't edit them by
hand.

Upstream project: <https://github.com/redswoop/xfer>
